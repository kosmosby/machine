...
class LanguageController extends acymController
{
    ...
    public function saveLanguage($fromShare = false)
    {
        ...
        if (!empty($customcontent)) {
            $customKeys = array();
            preg_match_all('#([0-9A-Z_]+)="((?:[^"]|"_QQ_")+)"#is', $customcontent, $customKeys);

            if (!empty($customKeys)) {
                $mainKeys = array();
                preg_match_all('#([0-9A-Z_]+)="((?:[^"]|"_QQ_")+)"#is', $content, $mainKeys);

                foreach ($customKeys[1] as $index => $oneKey) {
                    $position = array_search($oneKey, $mainKeys[1]);
                    if ($position !== false) {
                        $content = str_replace($mainKeys[0][$position], $customKeys[0][$index], $content);
                    } else {
