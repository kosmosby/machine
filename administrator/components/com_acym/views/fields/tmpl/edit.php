                ...
                <?php if (empty($data['field']->id) || $data['field']->id != 2) { ?>
                    <div class="cell margin-top-1 large-11" id="required_error_message">
                        <label class="acym__fields__change" id="acym__fields__error-message"><?php echo acym_translation('ACYM_CUSTOM_ERROR') ?>
                            <input type="text" name="field[option][error_message]" value="<?php echo empty($data['field']->option->error_message) ? '' : htmlspecialchars($data['field']->option->error_message, ENT_QUOTES, 'UTF-8'); ?>" placeholder="<?php echo htmlspecialchars(acym_translation_sprintf('ACYM_DEFAULT_REQUIRED_MESSAGE', 'xxx'), ENT_QUOTES, 'UTF-8'); ?>">
                        </label>
                    </div>
                <?php } ?>
